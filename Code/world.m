...
function buildWorld()
   ...
   % Define simulation actions and dynamics
    ...
    layers = [ ...
        ...
        fullyConnectedLayer(60, "Name", "myFullyConnectedLayer3")
        reluLayer("Name", "myReLu3")
...
function xdot = dynamics(x0, a)
    ...
    beta = atan(l_r * tan(delta)/L); % sideslip angle
    xdot = [v*cos(deg2rad(x0(3))+beta); v*sin(deg2rad(x0(3))+beta);(v/L)*tan(delta)*cos(beta)];
%     x1 = x0 + xdot*dt
...
function [r, isOffRoad] = getReward(state,scenario,goal)
    ...
    % Make Polygon form road boundaries to see if car is still on road
